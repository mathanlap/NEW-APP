...
class CRDSetup:
    def __init__(self, user):
        ...
        self.installQbit()
        self.install4KVideoDownloader()  # Add this line to install 4K Video Downloader
        self.finish(user)
    ...
    @staticmethod
    def install4KVideoDownloader():
        subprocess.run(['wget', 'https://dl.4kdownload.com/app/4kvideodownloaderplus_1.8.5-1_amd64.deb?source=website&_gl=1*1ay5m9h*_ga*Nzk1MDgzMjMzLjE3MjM0Nzc1MzI.*_ga_KRN0PZWTN9*MTcyMzQ3NzUzMi4xLjEuMTcyMzQ3OTIyMC4wLjAuMA', '-O', '4kvideodownloader.deb'])
        subprocess.run(['dpkg', '--install', '4kvideodownloader.deb'])
        subprocess.run(['apt', 'install', '--assume-yes', '--fix-broken'])
        print("4K Video Downloader Installed !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
